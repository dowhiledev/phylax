...
Examples:
  ...
  phylax scan "suspicious text"      Scan text against default policies
    ...
    parser.add_argument("--version", action="version", version=f"Phylax {__version__}")

    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Validate command
    validate_parser = subparsers.add_parser(
        "validate", help="Validate a policy configuration file"
    )
    validate_parser.add_argument(
        "config_file", type=Path, help="Path to the YAML policy configuration file"
    )

    # Scan command
    scan_parser = subparsers.add_parser("scan", help="Scan text against policies")
    scan_parser.add_argument("text", help="Text to scan for policy violations")
    scan_parser.add_argument(
        "--config", type=Path, help="Path to policy configuration file (optional)"
    )
